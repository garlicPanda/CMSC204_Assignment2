...
public class MyQueueTest {
	...
	public Double uno = 1.0, dos = 2.0, tres = 3.0, cuatro = 4.0, cinco = 5.0, seis = 6.0, siete = 7.0;
	@Before
	...
	public void testDequeueStudent() {
		...
		try {
			assertEquals(uno, doubleQ.dequeue());
			assertEquals(dos, doubleQ.dequeue());
			assertEquals(tres, doubleQ.dequeue());
	...
	public void testEnqueueStudent() {
		...
		try {
			...
			assertEquals(true, doubleQ.enqueue(cuatro));
			assertEquals(4, doubleQ.size());
			assertEquals(true, doubleQ.enqueue(cinco));
			assertEquals(5, doubleQ.size());
			assertEquals(true, doubleQ.enqueue(seis));
			assertEquals(6, doubleQ.size());
			
			doubleQ.enqueue(siete);
			assertTrue("This should have caused an QueueOverflowException", false);
		}catch(QueueOverflowException e) {
			assertTrue("This should have caused an QueueOverflowException", true);
		}catch(Exception e) {
	...
	public void testToStringStudent() {
		...
		doubleQ.enqueue(cuatro);
		assertEquals("1.02.03.04.0", doubleQ.toString());
		doubleQ.enqueue(cinco);
